...
export function ArchivesMinutesContent({ 
  ...
  const handleViewDocument = (minute: Minute) => {
    console.log('Attempting to view document:', minute);
    ...
    try {
      ...
      const base64Data = minute.fileData.trim();
      ...
      const blob = new Blob([byteArray], { type: minute.fileMimeType });
          ...
          <div>
                ...
                {filteredMinutes.map((minute, index) => (
                  ...
                  >
                    <td className="px-6 py-6">
                      <p className="text-[#101828] dark:text-gray-200">{minute.number}</p>
                    </td>
                    <td className="px-6 py-6">
                      <p className="text-[#4a5565] dark:text-gray-400 text-sm">{minute.recordId}</p>
                    </td>
                    <td className="px-6 py-6">
                      <div className="flex items-center gap-2">
                        <FileText className="w-4 h-4 text-[#3b7ffc] dark:text-blue-400" />
                        <p className="text-[#101828] dark:text-gray-200">{minute.title}</p>
                      </div>
                    </td>
                    <td className="px-6 py-6">
                      <div className="flex flex-col gap-1">
                        <p className="text-sm text-[#4a5565] dark:text-gray-400">
                          <span className="font-semibold">Author:</span> {minute.author}
                        </p>
                        <p className="text-sm text-[#4a5565] dark:text-gray-400">
                          <span className="font-semibold">Uploaded:</span> {minute.uploadedDate}
                        </p>
                        <p className="text-sm text-[#4a5565] dark:text-gray-400">
                          <span className="font-semibold">Last Edited by:</span> {minute.lastEditedBy}
                        </p>
                      </div>
                    </td>
                    <td className="px-6 py-6">
                      <p className="text-sm text-[#4a5565] dark:text-gray-400">
                        {minute.fileType} {minute.fileSize}
                      </p>
                    </td>
                    <td className="px-6 py-6">
                      <div className="flex items-center gap-2">
                        <button 
                          onClick={() => handleViewDocument(minute)}
            ...
            {filteredMinutes.length === 0 && (
              <div className="text-center py-12">
                <p className="text-gray-500 dark:text-gray-400">No minutes found matching your search.</p>
              </div>
            )}
          </div>
