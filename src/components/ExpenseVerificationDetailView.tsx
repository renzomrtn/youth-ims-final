...
function ApproveConfirmationModal({ isOpen, onClose, onConfirm, lineItemTitle }: ApproveConfirmationModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/30" role="dialog" aria-modal="true" aria-labelledby="modal-title">
      <div className="bg-white rounded-[24px] w-[500px] overflow-hidden relative shadow-2xl">
        <div className="bg-[#334870] h-[88px] flex items-center justify-between px-8">
          <p id="modal-title" className="font-['Source_Sans_3',sans-serif] font-bold text-[24px] text-white">
            Confirm Approval
          </p>
          <button 
            onClick={onClose} 
...
export function ExpenseVerificationDetailView({
  ...
  const handleFlagClick = () => {
    if (!isFlagDisabled) setIsFlaggingModalOpen(true);
  };

  const handleApproveConfirm = async () => {
    await updateExpenseItem(expenseItemId, { status: "Verified", flaggingData: null });
  ...
  const handleFlaggingConfirm = async (data: FlaggingData) => {
    ...
    await updateExpenseItem(expenseItemId, { status: "Flagged", flaggingData: enrichedFlaggingData });
    setSelectedExpenseIds([]);
  ...
  const handleFlaggedExpenseResponseConfirm = async (data: CorrectionData) => {
    if (particulars) {
      ...
      await updateExpenseItem(expenseItemId, { status: "Pending", flaggingData: null });
    }
    setSelectedExpenseIds([]);
  ...
  const getSelectedExpenses = () => expenseItems.filter((item) => selectedExpenseIds.includes(item.id));
  ...
  return (
    <div className="flex flex-col h-full bg-[#f3f3f3] dark:bg-gray-900">
      <div className="flex-1 overflow-auto p-8">
        <main className="bg-white dark:bg-gray-800 rounded-lg">
          ...
          <div className="p-8">
            <div className="mb-8">
              <h2 className="text-2xl text-black dark:text-white mb-6">{lineItemTitle}</h2>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex flex-col">
                      ...
                      {isOverBudget && (
                        <span className="text-[11px] font-bold text-red-600 dark:text-red-500 uppercase tracking-wider">Exceeds Budget</span>
                      )}
                    </div>
                  </div>
                  <div>
                ...
                <table className="w-full">
                    ...
                    <tr>
                      {(user?.role === "SKF Auditor" || user?.role === "SKF President") && status !== "Verified" && (
                        <th className="px-6 py-3 text-left text-gray-600 dark:text-gray-300 w-16">
                          <span className="sr-only">Select for flagging</span>
                        </th>
                      )}
                  ...
                  <tbody>
                    {expenseItems.length > 0 ? (
                        ...
                        <tr key={item.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                          {(user?.role === "SKF Auditor" || user?.role === "SKF President") && status !== "Verified" && (
                            <td className="px-6 py-4">
                              <label htmlFor={`check-${item.id}`} className="sr-only">Select {item.particular}</label>
                              <input
                                id={`check-${item.id}`}
                                type="checkbox"
                          ...
                          <td className="px-6 py-4">
                            {item.hasAttachment && (
                              <button 
                                aria-label={`View attachment for ${item.particular}`}
                      ...
                      <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-500 dark:text-gray-400">No expense items found</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <div className="w-96 space-y-4">
                ...
                <div className="flex justify-between items-center pb-4 border-b-2 border-gray-300 dark:border-gray-600">
                  <div className="flex flex-col">
                    ...
                    {isOverBudget && <span className="text-[10px] font-bold text-red-600 dark:text-red-500 uppercase tracking-wider">Exceeds Budget</span>}
                  </div>
                ...
                {user?.role === "SKF Treasurer" && status === "Flagged" && (
                  <div className="pt-4">
                    <button className="w-full h-[45px] bg-[#174499] hover:bg-[#0f2f6b] text-white rounded-[6px] transition-colors font-bold text-[14px]" onClick={() => setIsFlaggedExpenseResponseModalOpen(true)}>
                      Respond to Flag
                    </button>
                  </div>
                )}
                ...
                {(user?.role === "SKF Auditor" || user?.role === "SKF President") && status !== "Verified" && (
                  <div className="flex gap-3 pt-4">
                    <button
                      ...
                      className={`flex-1 h-[45px] text-white rounded-[6px] transition-all font-bold text-[14px] ${isFlagDisabled ? "bg-gray-400 cursor-not-allowed opacity-50" : "bg-[#ef4444] hover:bg-[#dc2626]"}`}
                    ...
                    <button
                      disabled={!isFlagDisabled} 
                      className={`flex-1 h-[45px] text-white rounded-[6px] transition-all font-bold text-[14px] ${!isFlagDisabled ? "bg-gray-400 cursor-not-allowed opacity-50" : "bg-[#174499] hover:bg-[#0f2f6b]"}`}
