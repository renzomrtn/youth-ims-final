...
interface ExpenseVerificationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (data: ExpenseVerificationData) => void;
}
...
// --- SECONDARY MODAL COMPONENT (Based on File 2 Design) ---
interface ConfirmationModalProps {
  data: ExpenseVerificationData;
  onConfirm: () => void;
  onBack: () => void;
  onClose: () => void;
  isSaving: boolean;
...
function ExpenseConfirmationView({ data, onConfirm, onBack, onClose, isSaving }: ConfirmationModalProps) {
  const totalAmount = data.particulars.reduce((sum, p) => sum + (p.amount || 0), 0);
  const isOverBudget = totalAmount > data.budget;
  ...
  return (
    <div className="bg-white rounded-[24px] w-[788px] max-h-[90vh] overflow-y-auto relative shadow-2xl">
      <div className="sticky top-0 bg-[#334870] h-[88px] rounded-t-[24px] z-10">
        <p className="absolute font-['Source_Sans_3',sans-serif] font-bold left-[40px] text-[24px] text-white top-[27px]">
          Confirm Expense Verification
        </p>
        <button onClick={onClose} className="absolute left-[728px] size-[24px] top-[32px] hover:opacity-80 transition-opacity">
      ...
      <div className="p-[40px] pt-[20px]">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 flex items-start gap-3">
          <CheckCircle className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" />
          <p className="text-blue-800 text-sm">
            Please review the summarized details below before finalizing. You can go back to edit if anything is incorrect.
          </p>
        </div>

        <section className="mb-6">
          <h3 className="font-semibold text-[20px] mb-4 text-black">Verification Summary:</h3>
          <div className="bg-gray-50 rounded-lg p-4 space-y-3">
            <div className="grid grid-cols-[160px_1fr] gap-2">
              <span className="font-semibold text-[14px] text-gray-700">Line Item:</span>
              <span className="text-[14px] text-black">{data.lineItem}</span>
            </div>
            <div className="grid grid-cols-[160px_1fr] gap-2">
              <span className="font-semibold text-[14px] text-gray-700">Budget:</span>
              <span className="text-[14px] text-black">₱{data.budget.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
            </div>
            <div className="grid grid-cols-[160px_1fr] gap-2">
              <span className="font-semibold text-[14px] text-gray-700">Period:</span>
              <span className="text-[14px] text-black">{data.expenditurePeriod.from} to {data.expenditurePeriod.to}</span>
            </div>
          </div>
        </section>

        <section className="mb-6">
          <h3 className="font-semibold text-[20px] mb-4 text-black">Particulars ({data.particulars.length}):</h3>
          <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
            {data.particulars.map((p) => (
              <div key={p.id} className="p-4 border border-gray-200 rounded-lg bg-white flex justify-between items-center">
                <div>
                  <p className="font-bold text-[#174499] text-[16px]">{p.description}</p>
                  <p className="text-[13px] text-gray-500">Date: {p.dateOfExpense}</p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-[16px]">₱{p.amount.toLocaleString()}</p>
                  <p className={`text-[11px] ${p.hasAttachment ? 'text-green-600' : 'text-amber-600'}`}>
                    {p.hasAttachment ? '✓ Receipt Attached' : '! No Attachment'}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className={`p-4 rounded-lg border-2 flex justify-between items-center mb-8 ${isOverBudget ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}>
          <span className="font-bold text-gray-700">Total Expenditure:</span>
          <div className="text-right">
            <p className={`text-[28px] font-bold ${isOverBudget ? 'text-red-600' : 'text-black'}`}>
              ₱{totalAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
            </p>
            {isOverBudget && <p className="text-[10px] text-red-500 font-bold uppercase">Exceeds Budget</p>}
          </div>
        </div>

        <div className="flex justify-end gap-[16px]">
          <button onClick={onClose} className="bg-[rgba(224,108,110,0.2)] border border-[#e06c6e] h-[45px] px-8 rounded-[6px] font-black text-[#e06c6e] text-[14px] hover:bg-[rgba(224,108,110,0.3)]">
            Cancel
          </button>
          <button onClick={onBack} className="bg-[rgba(172,172,172,0.2)] border border-[#acacac] h-[45px] px-8 rounded-[6px] font-black text-[#606060] text-[14px] hover:bg-[rgba(172,172,172,0.3)]">
            Previous
          </button>
          <button onClick={onConfirm} disabled={isSaving} className="bg-[#174499] h-[45px] px-8 rounded-[6px] font-black text-white text-[14px] hover:bg-[#0f2f6b]">
...
// --- MAIN COMPONENT ---
export function ExpenseVerificationModal({ isOpen, onClose, onConfirm }: ExpenseVerificationModalProps) {
  ...
  const [tempData, setTempData] = useState<ExpenseVerificationData | null>(null);
  ...
  const handleInitialConfirm = () => {
    ...
    setTempData({
      ...
      particulars: particulars.map(p => ({ ...p, hasAttachment: !!p.receipt }))
    });
  ...
  const handleFinalConfirm = async () => {
    ...
    try {
      ...
      alert('Failed to prepare expense verification. Please try again.');
      setIsSaving(false);
    }
  };
        ...
        <div className="bg-white rounded-[24px] w-[788px] max-h-[90vh] overflow-y-auto relative shadow-2xl">
          ...
          <div className="p-[40px] pt-[15px]">
            <div className="mb-[23px]">
              <p className="font-['Source_Sans_3',sans-serif] font-normal text-[16px] text-black mb-[8px]">Line Item:</p>
              <div className="relative" ref={lineItemDropdownRef}>
                <input
                  ...
                  disabled={isSaving}
                />
                <svg className="absolute right-4 top-1/2 -translate-y-1/2 w-[13px] h-[13px] pointer-events-none" fill="none" viewBox="0 0 13 13">
                ...
                {isLineItemDropdownOpen && filteredLineItems.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#939393] rounded-[5px] shadow-lg z-20 max-h-[200px] overflow-auto">
                    {filteredLineItems.map((item) => (
                      <button key={item.id} onClick={() => handleLineItemSelect(item)} className="w-full px-[16px] py-[10px] text-left hover:bg-gray-100 font-['Source_Sans_3',sans-serif] text-[16px] text-black">
                        {item.id} - {item.name}
                      </button>
                    ))}
                  </div>
            ...
            <div className="mb-[23px]">
              <p className="font-['Source_Sans_3',sans-serif] font-normal text-[16px] text-black mb-[8px]">Budget:</p>
              <div className="w-full h-[38px] rounded-[5px] border border-[#939393] px-[16px] bg-[#eee] flex items-center text-[16px] text-[#515151]">
                ₱{budget.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
            </div>

            <div className="mb-[23px]">
              <p className="font-['Source_Sans_3',sans-serif] font-normal text-[16px] text-black mb-[8px]">Expenditure Period:</p>
              <div className="flex gap-[27px]">
                <div className="flex-1">
                  <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="w-full h-[38px] border border-[#939393] rounded-[5px] px-[16px] text-[#515151] outline-none" disabled={isSaving} />
                </div>
                <div className="flex-1">
                  <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="w-full h-[38px] border border-[#939393] rounded-[5px] px-[16px] text-[#515151] outline-none" disabled={isSaving} />
                </div>
              </div>
            </div>
            ...
            {particulars.map((particular, index) => (
              <div key={particular.id} className="mb-[15px] relative border-b border-gray-100 pb-4">
                ...
                <div className="flex gap-[27px] mb-[16px]">
                  <div className="flex-1">
                    <p className="text-[16px] text-black mb-[8px]">Particulars:</p>
                    <input
                      type="text"
                      value={particular.description}
                      onChange={(e) => handleParticularChange(index, 'description', e.target.value)}
                      className="w-full h-[38px] rounded-[5px] border border-[#939393] px-[16px] text-[#515151] outline-none"
                      placeholder="Enter particulars..."
                      disabled={isSaving}
                    />
                  </div>
                  <div className="flex-1">
                    <p className="text-[16px] text-black mb-[8px]">Amount:</p>
                    <div className="relative">
                      <input
                        ...
                        className="w-full h-[38px] rounded-[5px] border border-[#939393] pl-[30px] pr-[50px] text-[#515151] outline-none"
                        disabled={isSaving}
                      />
                      <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-[2px]">
                        <button onClick={() => incrementAmount(index)} className="w-[13px] h-[15px] hover:opacity-70" disabled={isSaving}>
                          <svg className="size-full" fill="none" viewBox="0 0 13 15"><path d={svgPaths.p133eaac0} fill="#6D798E" /></svg>
                        </button>
                        <button onClick={() => decrementAmount(index)} className="w-[13px] h-[15px] hover:opacity-70" disabled={isSaving}>
                          <svg className="size-full" fill="none" viewBox="0 0 13 15"><path d={svgPaths.p10583a50} fill="#6D798E" /></svg>
                        </button>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mb-[16px]">
                  <p className="text-[16px] text-black mb-[8px]">Date of Expense:</p>
            ...
            <button onClick={addParticular} className="w-full h-[38px] rounded-[5px] border border-dashed border-[#939393] mb-[15px] text-[#515151] hover:bg-gray-50 transition-colors" disabled={isSaving}>
            ...
            <div className="bg-[#fefeff] h-[71px] rounded-[5px] border border-[#717171] flex items-center justify-between px-[23px] mb-[28px]">
              <p className="font-semibold text-[16px] text-black">Total Amount Spent:</p>
              <div className="flex flex-col items-end">
                <p className={`text-[32px] ${isOverBudget ? 'text-red-600' : 'text-black'}`}>
                  ₱{totalAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </p>
                {isOverBudget && <span className="text-[10px] uppercase text-red-500 font-bold">Exceeds Budget</span>}
              </div>
            </div>

            <div className="flex justify-end gap-[16px]">
              <button onClick={handleClose} className="bg-[rgba(224,108,110,0.2)] h-[45px] px-8 rounded-[6px] border border-[#e06c6e] font-black text-[#e06c6e] text-[14px] hover:bg-[rgba(224,108,110,0.3)] transition-colors" disabled={isSaving}>
                Cancel
              </button>
              <button onClick={handleInitialConfirm} className="bg-[#174499] h-[45px] px-8 rounded-[6px] font-black text-[14px] text-white hover:bg-[#0f2f6b] transition-colors" disabled={isSaving}>
                Confirm
              </button>
            </div>
          </div>
...
export default function App() {
  ...
  const handleConfirm = async (data: ExpenseVerificationData) => {
    console.log('Final Expense data:', data);
    alert('Expense verified and saved!');
  };

  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <button onClick={() => setIsOpen(true)} className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700">
        Open Expense Verification
      </button>
      <ExpenseVerificationModal isOpen={isOpen} onClose={() => setIsOpen(false)} onConfirm={handleConfirm} />
    </div>
  );
}